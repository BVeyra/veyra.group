...
// ─── Report token: encodes a submission into a compact URL-safe payload so
// the emailed report can link to a persistent /report page with no database. ───
...
type ReportTokenPayload = {
  ...
  u: number; // units
  t: number; // teamSize
  pm: number; rt: number; mf: number; or: number; ah: number; rc: number; // option indexes
  pp: number[]; // pain point indexes
};
...
export function encodeReportToken(data: AuditLeadData): string {
  const payload: ReportTokenPayload = {
    v: 1,
    n: data.name, c: data.company, e: data.email,
    u: data.units, t: data.teamSize,
    pm: PM_SOFTWARE_OPTIONS.indexOf(data.pmSoftware),
    rt: RESPONSE_TIME_OPTIONS.indexOf(data.responseTime),
    mf: MAINTENANCE_FLOW_OPTIONS.indexOf(data.maintenanceFlow),
    or: OWNER_REPORTING_OPTIONS.indexOf(data.ownerReporting),
    ah: AFTER_HOURS_OPTIONS.indexOf(data.afterHours),
    rc: RENT_COLLECTION_OPTIONS.indexOf(data.rentCollection),
    pp: data.painPoints.map((pain) => PAIN_OPTIONS.indexOf(pain)).filter((i) => i >= 0),
  };
  return toBase64Url(JSON.stringify(payload));
}

export function decodeReportToken(token: string): AuditLeadData | null {
  try {
    const raw = JSON.parse(fromBase64Url(token)) as ReportTokenPayload;
    if (!raw || raw.v !== 1) return null;
    const pmSoftware = PM_SOFTWARE_OPTIONS[raw.pm];
    const responseTime = RESPONSE_TIME_OPTIONS[raw.rt];
    const maintenanceFlow = MAINTENANCE_FLOW_OPTIONS[raw.mf];
    const ownerReporting = OWNER_REPORTING_OPTIONS[raw.or];
    const afterHours = AFTER_HOURS_OPTIONS[raw.ah];
    const rentCollection = RENT_COLLECTION_OPTIONS[raw.rc];
    if (!pmSoftware || !responseTime || !maintenanceFlow || !ownerReporting || !afterHours || !rentCollection) return null;
    const painPoints = (Array.isArray(raw.pp) ? raw.pp : [])
      .map((i) => PAIN_OPTIONS[i])
      .filter((pain): pain is PainOption => Boolean(pain));
    const units = Number(raw.u);
    const teamSize = Number(raw.t);
    if (!Number.isFinite(units) || !Number.isFinite(teamSize) || units <= 0 || teamSize <= 0) return null;
    if (typeof raw.n !== "string" || typeof raw.c !== "string" || typeof raw.e !== "string") return null;
    if (painPoints.length === 0) return null;
    return {
      name: raw.n, company: raw.c, email: raw.e,
      units: Math.round(units), teamSize: Math.round(teamSize),
      pmSoftware, responseTime, maintenanceFlow, ownerReporting, afterHours, rentCollection,
      painPoints,
    };
  } catch {
    return null;
