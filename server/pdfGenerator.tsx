...
export async function generateAndEmailPDF(data: AuditLeadData, insights: AuditInsights) {
  ...
  const text = [`${data.name},`, "", `Your preliminary PMS Operations Snapshot for ${data.company} is ready.`, `View your Snapshot: ${reportUrl}`, "", `Where to start: ${insights.likelyFocus.title}.`, "Preliminary and based on what you reported. A 15-minute Fit Call is where it gets tested.", "", insights.likelyFocus.description, "", `Book a 15-minute Fit Call: ${bookingUrl}`, "Prefer to talk now? Call (220) 244-4213."].join("\n");
